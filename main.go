...
func main() {
	...
	if operators_count > 1 {
		panic("Ошибка, ввод содержит более одного оператора") //а в чем отличие от того что было у меня с PrintLn и exit ? ))
		//fmt.Println("Ошибка, ввод содержит более одного оператора")
		//os.Exit(38)
	} else if operators_count == 0 {
		panic("Ошибка, ввод не содержит ни одного оператора")
	...
	for _, operator := range operators {
		if strings.HasPrefix(input, operator) || strings.HasSuffix(input, operator) {
			panic("Ошибка, оператор должен стоять между числами а не в начале или конце строки")
	...
	if strings.Trim(sliced_input[0], arabic_numbers_input) == "" && strings.Trim(sliced_input[1], arabic_numbers_input) == "" {
		...
		panic("Ошибка, ввод содержит недопустимые символы или вы пытаетесь оперировать числами двух разных систем записи (арабские с римскими)")
	...
	if number_1 < 1 || number_2 < 1 || number_1 > 10 || number_2 > 10 {
		panic("Ошибка, ввод содержит число меньше единицы или больше десяти")
	...
	case contains_roman_numbers:
		if result < 1 {
			panic("Ошибка, результат меньше единицы, римская система записи не поддерживает такие числа")
